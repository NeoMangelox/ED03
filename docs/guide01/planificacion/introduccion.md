...
## Descripción
El proyecto plantea la implementación de un software instalado en una computadora local que servirá como servidor de base de datos, conectado a dispositivos móviles o terminales utilizados por mozos y cocina. El **alcance del Producto Mínimo Viable (MVP)** se limita a las siguientes funcionalidades principales:
...
## Presupuesto
El proyecto considera un presupuesto estimado para cubrir el desarrollo del **alcance del MVP**:
...
## Tiempo
Se estima un tiempo de **13 semanas** para el desarrollo y entrega del **Producto Mínimo Viable (MVP)**, dividido en:
1.  **Análisis y diseño (enfocado en MVP)**: 4 semanas.
2.  **Desarrollo e integración (flujo central)**: 5 semanas.
3.  **Pruebas y validación (del MVP)**: 2 semanas.
4.  **Entrega del MVP + documentación final**: 1 semana.
...
- El **alcance inicial (MVP) no incluye funcionalidades avanzadas** como reportes de ventas, gestión de usuarios con roles, modificación de pedidos o control de stock, las cuales están planificadas como mejoras futuras.
